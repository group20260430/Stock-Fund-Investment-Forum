...
from sqlalchemy import Integer, Enum, ForeignKey, String, TIMESTAMP, func
...
class Certification(Base):
    ...
    status: Mapped[CertificationStatus] = mapped_column(
        Enum(CertificationStatus), default=CertificationStatus.PENDING, nullable=False, index=True
    )
    ...
    def __repr__(self) -> str:
        return f"<Certification(id={self.id}, user_id={self.user_id}, status={self.status.value})>"
